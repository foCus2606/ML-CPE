...
#    print(f"number of feature : {X_train.shape[1]}  ->  {data['feature_names']}")
   ...
   # ดึงเฉพาะรายชื่อคลาสที่มีการใช้งานจริงใน y_test หรือ y_pred
    unique_classes = np.unique(np.concatenate([y_test, y_pred]))
    active_class_names = [class_names[i] for i in unique_classes]

    evaluate.print_report(y_test, y_pred, active_class_names)

    cm = evaluate.plot_confusion_matrix(y_test, y_pred, active_class_names,
